...
namespace ZOOM_SDK_DOTNET_WRAP {
	...
	public:
		...
		void onUserNamesChanged(ZOOM_SDK_NAMESPACE::IList<unsigned int>* lstUserID)
		{
			if (CMeetingParticipantsControllerDotNetWrap::Instance)
				CMeetingParticipantsControllerDotNetWrap::Instance->procUserNamesChanged(Convert(lstUserID));
		}
	private:
	...
	void CMeetingParticipantsControllerDotNetWrap::BindEvent()
	{
		...
		meetingParticipants.m_cbonUserNamesChanged =
			std::bind(&ParticipantsEventHandler::onUserNamesChanged,
				&ParticipantsEventHandler::GetInst(), std::placeholders::_1);
	}
	...
	void CMeetingParticipantsControllerDotNetWrap::procUserNamesChanged(array<unsigned int >^ lstUserID)
	{
		event_onUserNamesChanged(lstUserID);
	}
	...
	SDKError CMeetingParticipantsControllerDotNetWrap::LowerAllHands(bool forWebinarAttendees)
	{
		return (SDKError)ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().
			GetMeetingParticipantsController().LowerAllHands(forWebinarAttendees);
	}
