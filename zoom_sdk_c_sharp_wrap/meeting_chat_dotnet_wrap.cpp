...
namespace ZOOM_SDK_DOTNET_WRAP {
	...
	void CMeetingChatControllerDotNetWrap::BindEvent()
	{
		...
		meetingChat.m_cbonChatStatusChangedNotification =
	...
	SDKError CMeetingChatControllerDotNetWrap::SendChatMsgTo(String^ content, unsigned int receiver, ChatMessageType type)
	{
		ZOOM_SDK_NAMESPACE::IMeetingChatControllerWrap& meetingChat = ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().GetMeetingChatController();
		ZOOM_SDK_NAMESPACE::IChatMsgInfoBuilder* pChatMsgInfoBuilder = meetingChat.GetChatMessageBuilder();
		if (!pChatMsgInfoBuilder)
		{
			return (SDKError)ZOOM_SDK_NAMESPACE::SDKERR_INTERNAL_ERROR;
		}
		pChatMsgInfoBuilder->SetContent(const_cast<wchar_t*>(PlatformString2WChar(content)));
		pChatMsgInfoBuilder->SetReceiver(receiver);
		pChatMsgInfoBuilder->SetMessageType((ZOOM_SDK_NAMESPACE::SDKChatMessageType)type);
		...
		return (SDKError)ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().
			GetMeetingChatController().SendChatMsgTo(pChatMsgInfo);
	}

	bool CMeetingChatControllerDotNetWrap::IsMeetingChatLegalNoticeAvailable()
	{
		return ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().
			GetMeetingChatController().IsMeetingChatLegalNoticeAvailable();
	}

	String^ CMeetingChatControllerDotNetWrap::getChatLegalNoticesPrompt()
	{
		return WChar2PlatformString(ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().
			GetMeetingChatController().getChatLegalNoticesPrompt());
	}

	String^ CMeetingChatControllerDotNetWrap::getChatLegalNoticesExplained()
	{
		return WChar2PlatformString(ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().
			GetMeetingChatController().getChatLegalNoticesExplained());
	}
