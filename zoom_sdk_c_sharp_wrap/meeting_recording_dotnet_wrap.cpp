...
namespace ZOOM_SDK_DOTNET_WRAP {
	...
	SDKError CMeetingRecordingControllerDotNetWrap::StartRecording(DateTime^% startTimestamp)
	{
		time_t time_c;
		SDKError err = (SDKError)ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().
			GetMeetingRecordingController().StartRecording(time_c);
	...
	void CMeetingRecordingControllerDotNetWrap::BindEvent()
	{
		...
		meetingRec.m_cbonRecordingStatus =
			std::bind(&MeetingRecordingControllerEventHandler::onRecordingStatus,
				&MeetingRecordingControllerEventHandler::GetInst(), std::placeholders::_1);
		meetingRec.m_cbonRecordPrivilegeChanged =
			std::bind(&MeetingRecordingControllerEventHandler::onRecordPrivilegeChanged,
				&MeetingRecordingControllerEventHandler::GetInst(), std::placeholders::_1);
	...
	{
		event_onRecordPrivilegeChanged(bCanRec);
	}
