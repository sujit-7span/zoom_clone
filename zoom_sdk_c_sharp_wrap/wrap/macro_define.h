...
#define INIT_UNINIT_OWNSERVICE(Classname, OwnerClass)\
	void Init_Wrap(OwnerClass* pOwner){m_obj = Init##Classname##Func(pOwner);};\
	void Uninit_Wrap(){Uninit##Classname##Func(m_obj);m_obj=NULL;};
...
#define CallBack_FUNC_Override_2(funcname, T1, P1, T2, P2)\
virtual void funcname(T1 P1, T2 P2)\
{\
	if (NULL != m_cboverride##funcname)\
		m_cboverride##funcname(P1, P2);\
	if (NULL != external_cb)\
		external_cb->funcname(P1, P2);\
}\
std::function<void(T1, T2)> m_cboverride##funcname;
